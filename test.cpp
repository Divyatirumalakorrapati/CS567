...
TEST(HangmanLib, CheckGuess) {
    ...
    std::cout << "Testing guess for secret word: " << secretWord << std::endl;
    
    // Test correct guess
    ASSERT_TRUE(checkGuess(secretWord[0], secretWord));

    // Test incorrect guess
    ASSERT_FALSE(checkGuess('z', secretWord));  // Assuming 'z' is unlikely to be in the word
}
...
TEST(HangmanLib, SecretWordLength) {
    std::string secretWord = generateRandomSecretWord();
    std::cout << "Generated secret word: " << secretWord << std::endl;
