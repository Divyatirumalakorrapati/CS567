...
#define FRUITS_LEN 5
#define ATTEMPTS_EASY 10
#define ATTEMPTS_MEDIUM 7
#define ATTEMPTS_HARD 5

const std::string FRUITS[] = {"apple", "banana", "cherry", "mango", "orange"};
...
void playHangman(const std::vector<std::string>& playerNames) {
    ...
    while (playAgain) {
        ...
        for (const std::string& playerName : playerNames) {
            ...
            // Final outcome for the player
            if (checkVictory(outputArray)) {
                std::cout << "\nCongratulations, " << playerName << "! You guessed the word: " << secretWord << "\n";
                scores[&playerName - &playerNames[0]]++;
            } else {
                std::cout << "\nSorry, " << playerName << ", you lost. The secret word was: " << secretWord << "\n";
            }
        }
    ...
    for (size_t i = 0; i < playerNames.size(); ++i) {
        std::cout << playerNames[i] << ": " << scores[i] << " points\n";
    }
}
